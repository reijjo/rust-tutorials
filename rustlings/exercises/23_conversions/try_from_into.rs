...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        let r = if (0..=255).contains(&tuple.0) { tuple.0 as u8 } else { return Err(IntoColorError::IntConversion); };
        let g = if (0..=255).contains(&tuple.1) { tuple.1 as u8 } else { return Err(IntoColorError::IntConversion); };
        let b = if (0..=255).contains(&tuple.2) { tuple.2 as u8 } else { return Err(IntoColorError::IntConversion); };
